				...
				debugger;
						...
						for(var prop in mItem){
							...
							if(ctx.keystoneModel.fields[prop]){
								...
								*/
								if(mItem[prop]){
									if(ctx.keystoneModel.fields[prop]=='localfile'){
										ctx.keystoneModel.fields[prop].uploadfile('filedata', false, function(){});
									}
									doc[prop]=mItem[prop];
									doSave=true;
								}
						...
						if(doSave){
							if(update){
								...
								ctx.keystoneModel.model.update(q, doc, {upsert: true},function(err, num){
console.log(num +" updated documents on :"+JSON.stringify(q));
								});
							}
							else{
								doc.save();
		...
		getKeystoneItem:function(mItem){
			...
			if(this.mapKey && mItem[this.mapKey]){
				this.keystoneModel.model.findOne(new Object()[this.mapKey]=mItem[this.mapKey])
